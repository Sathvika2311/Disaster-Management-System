...
// Active emergencies: active or responding
$activeSql = "SELECT COUNT(*) AS count FROM incidents WHERE status IN ('active', 'responding')";
$activeResult = $conn->query($activeSql);
$active = ($activeResult && $row = $activeResult->fetch_assoc()) ? $row['count'] : 0;

// Resolved cases: resolved or closed
$resolvedSql = "SELECT COUNT(*) AS count FROM incidents WHERE status IN ('resolved', 'closed')";
$resolvedResult = $conn->query($resolvedSql);
$resolved = ($resolvedResult && $row = $resolvedResult->fetch_assoc()) ? $row['count'] : 0;

// Pending cases: pending or reported
$pendingSql = "SELECT COUNT(*) AS count FROM incidents WHERE status IN ('pending', 'reported')";
$pendingResult = $conn->query($pendingSql);
$pending = ($pendingResult && $row = $pendingResult->fetch_assoc()) ? $row['count'] : 0;

// Available resource units
$resourceSql = "SELECT COUNT(*) AS count FROM resources WHERE status = 'available'";
$resourceResult = $conn->query($resourceSql);
$available_resources = ($resourceResult && $row = $resourceResult->fetch_assoc()) ? $row['count'] : 0;
    ...
    <main>
        ...
        <div class="container mx-auto px-4 py-8">
            ...
            <div class="mb-8">
                ...
                <p class="text-gray-400 mb-6">Welcome, <?php echo htmlspecialchars($currentUser['first_name']); ?> (Administrator)</p>
                    ...
                    <div class="bg-gray-900 p-4 rounded-lg border border-gray-800">
                        <div class="flex items-center">
                            ...
                            <div class="ml-4">
                                ...
                                <h3 class="text-xl font-bold text-white" id="activeEmergencies"><?php echo $active; ?></h3>
                            </div>
                        </div>
                    </div>
                    <div class="bg-gray-900 p-4 rounded-lg border border-gray-800">
                        <div class="flex items-center">
                            ...
                            <div class="ml-4">
                                ...
                                <h3 class="text-xl font-bold text-white" id="resolvedCases"><?php echo $resolved; ?></h3>
                            </div>
                        </div>
                    </div>
                    <div class="bg-gray-900 p-4 rounded-lg border border-gray-800">
                        <div class="flex items-center">
                            ...
                            <div class="ml-4">
                                ...
                                <h3 class="text-xl font-bold text-white" id="pendingCases"><?php echo $pending; ?></h3>
                            </div>
                        </div>
                    </div>
                    <div class="bg-gray-900 p-4 rounded-lg border border-gray-800">
                        <div class="flex items-center">
                            ...
                            <div class="ml-4">
                                <p class="text-gray-400 text-sm">Available Resources</p>
                                <h3 class="text-xl font-bold text-white" id="availableUnits"><?php echo $available_resources; ?></h3>
                            </div>
                        </div>
                    </div>
                </div>
                            ...
                            <table class="min-w-full">
                                ...
                                <tbody id="emergencyList" class="text-gray-300">
                                    ...
                                    $sql = "SELECT i.id, i.title, i.description, i.location, i.latitude, i.longitude, 
                                            ...
                                            CONCAT(u.first_name, ' ') as reporter_name, u.id as reporter_id
                                            FROM incidents i 
                ...
                <div class="space-y-8">
                    ...
                    <div class="bg-gray-900 rounded-lg border border-gray-800 p-6">
                        ...
                        <div class="space-y-2">
							...
							<button onclick="window.location.href='signup.php'" class="w-full bg-red-600 text-white px-4 py-2 rounded-lg hover:bg-red-700 transition-colors">
                                <i class="fas fa-plus plus-icon"></i> Add Reporters
                            </button>
							<button onclick="window.location.href='view_user.php'" class="w-full bg-red-600 text-white px-4 py-2 rounded-lg hover:bg-red-700 transition-colors">
                                <i class="fas fa-eye"></i> View Reporters
                            </button>
                        </div>
                    </div>
