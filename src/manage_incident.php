    ...
    case 'approve':
        ...
        if ($incident['status'] == 'pending' || $incident['status'] == 'reported') {
            ...
            if ($update_stmt->execute()) {
                ...
                $_SESSION['success'] = "Incident approved and marked as active.";
            } else {
    ...
    case 'reject':
        ...
        if ($incident['status'] == 'pending' || $incident['status'] == 'reported') {
            ...
            if ($update_stmt->execute()) {
                ...
                $_SESSION['success'] = "Incident has been rejected.";
            } else {
    ...
    case 'dispatch':
        ...
        if ($incident['status'] == 'active') {
            if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['dispatch_resources'])) {
                ...
                if (isset($_POST['resources']) && is_array($_POST['resources']) && !empty($_POST['resources'])) {
                    ...
                    try {
                        ...
                        foreach ($_POST['resources'] as $resource_id) {
                            ...
                            $update_resource_sql = "UPDATE resources SET status = 'available', assigned_to = ? WHERE id = ? AND status = 'available'";
                        ...
                        // Success message
                        if ($assigned_count > 0) {
                            $_SESSION['success'] = "Response team dispatched. {$assigned_count} resources assigned.";
                ...
                $page_title = "Dispatch Resources - Incident ";
                ...
                <div class="container mx-auto px-4 py-8">
                    ...
                    <div class="mb-6">
                        <a href="view_incident.php?id=<?php echo $incident_id; ?>" class="text-blue-500 hover:underline">
                            <i class="fas fa-arrow-left mr-2"></i>Back to Disaster
                        </a>
                    </div>
                    
                    <div class="bg-gray-900 rounded-lg border border-gray-800 p-6">
                        <h2 class="text-xl font-bold text-white mb-6">Dispatch Resources to Disaster</h2>
                        
                        <div class="mb-6">
                            <h3 class="text-lg font-medium text-white mb-3">Disaster Details</h3>
...
if (!empty($incident['additional_data'])) {
    ...
    if (!empty($additional_data['resources']) && is_array($additional_data['resources'])) {
        $resources_list = implode(', ', array_map('htmlspecialchars', $additional_data['resources']));
    }
}
?>
...
<div>
    <strong>Resources needed:</strong> <?php echo $resources_list; ?>
</div>
<?php endif; ?>
    ...
    <h3 class="text-lg font-medium text-white mb-6">Select Resources to Dispatch</h3>
    ...
    case 'resolve':
        ...
        if ($incident['status'] == 'responding' || $incident['status'] == 'active') {
            ...
            if ($update_stmt->execute()) {
                ...
                $_SESSION['success'] = "Incident has been resolved.";
            } else {
    ...
    case 'delete':
        ...
        if ($delete_stmt->execute()) {
            $_SESSION['success'] = "Incident has been deleted.";
        } else {
