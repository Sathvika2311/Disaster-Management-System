...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($_POST['first_name']) || empty($_POST['email']) || empty($_POST['phone'])) {
        ...
        if (empty($error_message)) {
            if ($password_updated) {
                $update_query = "UPDATE users SET first_name = ?, email = ?, phone = ?, password = ? WHERE id = ?";
                $stmt = $conn->prepare($update_query);
                $stmt->bind_param("ssssi", $first_name, $email, $phone, $new_password_hash, $currentUser['id']);
            } else {
                $update_query = "UPDATE users SET first_name = ?, email = ?, phone = ? WHERE id = ?";
                $stmt = $conn->prepare($update_query);
                $stmt->bind_param("sssi", $first_name, $email, $phone, $currentUser['id']);
            }
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div class="md:col-span-2">
                <div class="bg-gray-900 rounded-lg border border-gray-800 p-6">
                    ...
                    <form method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" class="space-y-6">
                            ...
                            <!-- First Name -->
                            <div>
                                <label for="first_name" class="block text-gray-300 mb-2">Name <span class="text-red-500">*</span></label>
                        ...
                        <!-- Phone -->
                        <div>
                            <label for="phone" class="block text-gray-300 mb-2">Phone Number <span class="text-red-500">*</span></label>
                            <input type="text" id="phone" name="phone" value="<?php echo htmlspecialchars($currentUser['phone'] ?? ''); ?>" required class="w-full px-4 py-2 rounded-lg bg-gray-800 border border-gray-700 text-white focus:outline-none focus:border-blue-500">
                        </div>
            ...
            <div>
                <div class="bg-gray-900 rounded-lg border border-gray-800 p-6">
                    ...
                    <h3 class="text-lg font-bold text-white text-center mb-4"><?php echo htmlspecialchars($currentUser['first_name'] . ' '); ?></h3>
                    ...
                    <div class="space-y-3">
                        <div class="flex items-center">
						 ...
						 <div class="flex items-center">
                            ...
                            <span class="text-gray-300"> <?php echo htmlspecialchars($currentUser['location']); ?></span>
                        </div>
